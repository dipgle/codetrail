...
const I18N = {
  en: {
    ...
    "price.title": "Pricing",
    "price.lede": "Codetrail is in open alpha. The free tier — local devlog, hooks, browser viewer, UC/TC tracking — is MIT-licensed and ships today. Pricing for cloud sync (Pro / Team tiers) is being finalized; nothing is committed yet. Join the waitlist below to be notified when paid tiers open.",
    ...
    "wait.lede": "Codetrail is in open alpha. The free tier ships today (clone the repo or run the install script); the cloud-sync tier and the MCP binary download are still in progress. Drop your email and we'll send a note when paid tiers open.",
    ...
    "terms.s1p": "Codetrail provides hooks, a project scaffolder, and a web viewer that help you maintain persistent memory across AI coding sessions. An MCP server binary and paid cloud-sync tiers are on the roadmap. The free tier — including all local-only features — is available to anyone without registration.",
    ...
    "terms.s6p": "The landing site, hooks, and project template are MIT-licensed at github.com/dipgle/codetrail. The MCP server binary (project-agent) is closed source; a hosted binary download is on the roadmap and will ship under the binary EULA. You may not reverse-engineer, decompile, or redistribute the binary outside of permitted uses in the EULA.",
    ...
    "terms.s8p": "We may update these terms. Material changes (anything that meaningfully reduces your rights) will be announced with reasonable advance notice (target: 30 days where feasible) via the changelog and, for users who have provided an email, by email; minor edits (typos, clarifications) may be posted without notice. Continued use after the notice period means you accept the new terms. If you don't, cancel before the effective date and we'll honor the old terms for any unused paid period.",
    ...
    "priv.s4l5": "jsDelivr CDN (Fastly / Cloudflare edge) — serves the public sql.js WebAssembly bundle to the browser viewer. Your IP and User-Agent reach the CDN for the asset request only; the SQLite file you drop in stays in your browser and is never sent anywhere.",
    "priv.s4ps": "We will update this list before adding a new sub-processor that handles personal data, with reasonable advance notice (target: 30 days where feasible). You can object by email; if we can't accommodate, you can cancel your plan and we'll refund any unused prepaid period.",
    "priv.s5": "5. Security",
  ...
  vi: {
    ...
    "price.lede": "Codetrail đang ở open alpha. Free tier — local devlog, hooks, browser viewer, UC/TC tracking — MIT-licensed và ship hôm nay. Giá cho cloud sync (Pro / Team) đang được hoàn thiện; chưa cam kết gì. Đăng ký waitlist dưới đây để được thông báo khi gói trả phí mở.",
    ...
    "wait.lede": "Codetrail đang ở open alpha. Free tier ship hôm nay (clone repo hoặc chạy install script); cloud-sync tier và MCP binary download vẫn đang làm. Để lại email, chúng tôi gửi ghi chú khi gói trả phí mở.",
    ...
    "terms.s1p": "Codetrail cung cấp hook, project scaffolder, và web viewer giúp bạn duy trì memory bền vững giữa các phiên AI coding. MCP server binary và gói cloud-sync trả phí nằm trong roadmap. Free tier — bao gồm mọi tính năng local-only — miễn phí cho mọi người, không cần đăng ký.",
    ...
    "terms.s6p": "Landing, hook, và project template đều MIT tại github.com/dipgle/codetrail. MCP server binary (project-agent) closed source; hosted binary download nằm trong roadmap và sẽ ship dưới binary EULA. Không reverse-engineer, decompile, hoặc phát tán binary ngoài phạm vi EULA cho phép.",
    "terms.s7": "7. Chấm dứt",
    ...
    "terms.s8p": "Chúng tôi có thể cập nhật. Thay đổi material (làm giảm quyền của bạn) sẽ được thông báo với thời gian báo trước hợp lý (mục tiêu: 30 ngày khi khả thi) qua changelog và, đối với user đã cung cấp email, qua email; sửa nhỏ (typo, làm rõ) có thể post không cần notice. Tiếp tục dùng sau notice = chấp nhận điều khoản mới. Nếu không, hủy trước ngày hiệu lực và chúng tôi tôn trọng điều khoản cũ cho phần đã trả chưa dùng.",
    ...
    "priv.s4l5": "jsDelivr CDN (Fastly / Cloudflare edge) — phục vụ bundle sql.js WebAssembly công khai cho viewer trình duyệt. IP và User-Agent của bạn đến CDN chỉ cho request asset; file SQLite bạn thả vào ở lại trong trình duyệt và không bao giờ được gửi đi đâu.",
    "priv.s4ps": "Chúng tôi cập nhật list này trước khi thêm sub-processor mới xử lý personal data, với thời gian báo trước hợp lý (mục tiêu: 30 ngày khi khả thi). Bạn có thể phản đối qua email; nếu không accommodate được, bạn có thể hủy plan và chúng tôi refund phần prepaid chưa dùng.",
    "priv.s5": "5. Bảo mật",
