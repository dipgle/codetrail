...
const I18N = {
  en: {
    "meta.title": "Codetrail — Persistent memory for Claude Code sessions",
    ...
    "box.title": "What lands on your machine",
    "box.lede": "One install, six pieces. All MIT, all readable, all on your disk — nothing phones home.",
    "box.devlog.title": "The devlog + MCP server",
    "box.devlog.body": "A SQLite file per project and a server exposing 14 tools over it. Ships in Node and Rust — same tools, pick by whichever toolchain you already have.",
    "box.hooks.title": "Hooks",
    "box.hooks.body": "Auto-artifact logging, stale-resume warning, secret scrubbing, source logging. You copy them into ~/.claude/hooks/ yourself — the installer never touches that directory.",
    "box.scaffold.title": "The scaffolder",
    "box.scaffold.body": "np for a new project, adopt for one that already has code. Both idempotent: they add what is missing and never overwrite what you wrote.",
    "box.runner.title": "The runner daemon",
    "box.runner.body": "A file-queue for when the assistant cannot reach a shell. Every command is checked against a text allowlist you own, runs in its own subshell, and lands in an audit log.",
    "box.monitor.title": "The terminal monitor",
    "box.monitor.body": "A read-only dashboard over the devlog, refreshing every 2s. Health warnings, active use cases, recent events, test summary — in a second terminal while the session works.",
    "box.viewer.title": "The browser viewer",
    "box.viewer.body": "Drag a devlog.sqlite onto the page and read it. Pure WASM — the file is parsed in the tab and never uploaded, so you can hand it to a teammate who installs nothing.",
    ...
    "about.who.trace.p": "An AI agent, Claude family. Trace writes the product, the landing, the legal pages, the deploy scripts, and the commits — including the one that shipped this page. Every git commit on github.com/dipgle/codetrail is authored by Trace.",
    ...
    "about.principles.p1": "Honest copy. If a feature isn't shipped yet, the page says \"coming soon\" — never present. Every capability listed on the landing is one you can read the source of in the repo.",
    ...
    "inst.lede": "Two helpers: np for new projects, adopt for existing ones. macOS and Linux natively; Windows through WSL2. Requires Node 20+ and Claude Code — the terminal monitor wants Node 22+.",
  ...
  vi: {
    "meta.title": "Codetrail — Bộ nhớ bền vững cho phiên Claude Code",
    ...
    "box.title": "Cài xong bạn có gì trong máy",
    "box.lede": "Một lần cài, sáu thành phần. Tất cả MIT, đọc được, nằm trên đĩa của bạn — không có gì gọi về nhà.",
    "box.devlog.title": "Devlog + MCP server",
    "box.devlog.body": "Mỗi project một file SQLite, kèm server mở 14 tool trên đó. Có cả bản Node và Rust — cùng bộ tool, chọn theo toolchain bạn sẵn có.",
    "box.hooks.title": "Hooks",
    "box.hooks.body": "Tự log artifact, cảnh báo session cũ, che chuỗi bí mật, ghi nguồn đã tra. Bạn tự chép vào ~/.claude/hooks/ — trình cài không bao giờ đụng thư mục đó.",
    "box.scaffold.title": "Bộ dựng project",
    "box.scaffold.body": "np cho project mới, adopt cho project đã có code. Cả hai đều idempotent: chỉ thêm phần còn thiếu, không ghi đè thứ bạn đã viết.",
    "box.runner.title": "Runner daemon",
    "box.runner.body": "Hàng đợi bằng file, dùng khi trợ lý không với được shell. Mọi lệnh phải khớp allowlist dạng text do bạn giữ, chạy trong subshell riêng, và để lại vết trong audit log.",
    "box.monitor.title": "Monitor trên terminal",
    "box.monitor.body": "Bảng theo dõi chỉ-đọc trên devlog, làm mới mỗi 2 giây. Cảnh báo sức khoẻ, use case đang chạy, sự kiện gần nhất, tổng hợp test — mở ở terminal thứ hai trong lúc phiên làm việc.",
    "box.viewer.title": "Viewer trên trình duyệt",
    "box.viewer.body": "Kéo một file devlog.sqlite vào trang là đọc được. Thuần WASM — file được phân tích ngay trong tab, không upload đi đâu, nên đưa cho đồng nghiệp không cần cài gì.",
    ...
    "about.who.trace.h": "Trace — Lead Engineer",
    "about.who.trace.p": "Một AI agent thuộc dòng Claude. Trace viết sản phẩm, landing, legal page, deploy script, và commit — bao gồm commit ship trang này. Mọi git commit trên github.com/dipgle/codetrail đều do Trace tác giả.",
    ...
    "about.principles.p1": "Copy trung thực. Nếu feature chưa ship, trang ghi \"sắp có\" — không bao giờ giả vờ đã có. Mọi năng lực ghi trên landing đều đọc được mã nguồn trong repo.",
    ...
    "inst.lede": "Hai helper: np tạo project mới, adopt cho project có sẵn. macOS và Linux chạy thẳng; Windows đi qua WSL2. Yêu cầu Node 20+ và Claude Code — riêng monitor terminal cần Node 22+.",
